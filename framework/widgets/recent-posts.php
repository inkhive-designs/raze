...
class raze_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'raze_rp', // Base ID
			__('Raze Recent Posts with Thumbnails', 'raze'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'raze' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'raze' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'raze' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:','raze' ); ?></label> 
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:', 'raze' ); ?></label>
...
add_action( 'widgets_init', 'register_raze_widget' );  
function register_raze_widget() {  
    register_widget( 'raze_Recent_Posts' );  
}  
