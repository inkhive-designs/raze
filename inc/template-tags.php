...
function raze_paging_nav() {
	...
	<nav class="navigation paging-navigation" role="navigation">
		...
		<div class="nav-links">
			...
			<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'raze' ) ); ?></div>
			<?php endif; ?>

			<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'raze' ) ); ?></div>
			<?php endif; ?>
...
function raze_posted_on() {
	...
	$posted_on = sprintf(
		_x( 'Posted on %s', 'post date', 'raze' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		_x( 'by %s', 'post author', 'raze' ),
...
function raze_posted_on_icon( $param ) {
	$time_string = '<time class="entry-date published updated" datetime="%1$s">%2$s</time>';
	if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
		$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time><time class="updated" datetime="%3$s">%4$s</time>';
	}
	...
	$posted_on = sprintf(
		_x( '<i class="fa fa-calendar"></i> %s', 'post date', 'raze' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		_x( '<i class="fa fa-user"></i> %s', 'post author', 'raze' ),
		'<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></span>'
	);
...
if ( ! function_exists( 'raze_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function raze_entry_footer() {
	// Hide category and tag text for pages.
	if ( 'post' == get_post_type() ) {
		/* translators: used between list items, there is a space after the comma */
		$categories_list = get_the_category_list( __( ', ', 'raze' ) );
		if ( $categories_list && raze_categorized_blog() ) {
			printf( '<span class="cat-links">' . __( 'Posted in %1$s', 'raze' ) . '</span>', $categories_list );
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', __( ', ', 'raze' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . __( 'Tagged %1$s', 'raze' ) . '</span>', $tags_list );
		}
	}

	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( __( 'Leave a comment', 'raze' ), __( '1 Comment', 'raze' ), __( '% Comments', 'raze' ) );
		echo '</span>';
	}

	edit_post_link( __( 'Edit', 'raze' ), '<span class="edit-link">', '</span>' );
}
endif;
...
if ( ! function_exists( 'raze_comment' ) ) :
/**
 * Template for comments and pingbacks.
 *
 * Used as a callback by wp_list_comments() for displaying the comments.
 */
function raze_comment( $comment, $args, $depth ) {
	...
	<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
		<div class="comment-body">
			<?php _e( 'Pingback:', 'raze' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'raze' ), '<span class="edit-link">', '</span>' ); ?>
		</div>
	...
	<li id="comment-<?php comment_ID(); ?>" <?php comment_class( empty( $args['has_children'] ) ? '' : 'parent' ); ?>>
		<article id="div-comment-<?php comment_ID(); ?>" class="comment-body row">
				...
				<div class="comment-metadata col-md-10 col-sm-10 col-xs-12">
					...
					<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
						<time datetime="<?php comment_time( 'c' ); ?>">
							<?php printf( _x( '%1$s', '1: date', 'raze' ), get_comment_date() ); ?>
						</time>
					</a>
				</div><!-- .comment-metadata -->

				<?php if ( '0' == $comment->comment_approved ) : ?>
				<p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'raze' ); ?></p>
				<?php endif; ?>
			</footer><!-- .comment-meta -->
			...
			?>
			<?php edit_comment_link( __( 'Edit', 'raze' ), '<span class="edit-link">', '</span>' ); ?>
			</div><!-- .comment-content -->
...
endif; // ends check for raze_comment()
